        ...
        <div class="post-tag mycenter">
            ...
            if ($posttags) {
                if (count($posttags) === 1) {
                    ...
                    <a href="<?php echo get_term_link( intval($posttags[0]->term_id), $posttags[0]->taxonomy ) ?> "><i class="icon-tags icon-large"></i>
                        <?php echo $posttags[0]->name; ?>
                    </a>
                </span>
                <?php } else { ?>
                    <span class="post-tag-tags label label-danger  mylabel">
                        <?php foreach ($posttags as $posttag=>$tag){ ?>
                    <a href="<?php echo get_term_link( intval($tag->term_id), $tag->taxonomy ) ?>"><?php if($posttag===0){ ?><i class="icon-tags icon-large"></i> <?php } ?>
                        <?php echo $tag->name; ?>
                    </a>
                        <?php } ?>
                </span>
